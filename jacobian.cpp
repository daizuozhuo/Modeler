...
state(NOT_INIT) {}
Jacobian::Jacobian(int freedom): deg_freedom(freedom), state(NOT_INIT) {
	int i, j;
	for(i = 0; i < 4; i++) {
		for(j = 0; j < 4; j++) {
			rawTrans[i][j] = (i == j);
	...
	Jacob = new Mat<ExprP>(deg_freedom, 4);
...
void Jacobian::pushRotV(int varid, double x, double y, double z) {
	...
	rotMat[0][1] = y * y * (1.0 - c) + c;
	rotMat[0][2] = y * z * (1.0 - c) - x * s;
	rotMat[0][3] = 0;

	rotMat[1][0] = x * z * (1.0 - c) - y * s;
	rotMat[0][1] = y * z * (1.0 - c) + x * s;
	rotMat[0][2] = z * z * (1.0 - c) + c;
	rotMat[0][3] = 0;
